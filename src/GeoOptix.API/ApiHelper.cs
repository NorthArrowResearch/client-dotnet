...
namespace GeoOptix.API
{
    public class ApiHelper
    {
        ...
        public TokenResponse AuthToken { get; private set; }
        ...
        public ApiHelper(string fullUrl, TokenResponse authToken)
            : this(fullUrl)
        {
            AuthToken = authToken;
        }
        ...
        public ApiResponse<TransferSummary[]> GetTransfers()
        {
            var url = MakeTransferCollectionUrl(_url);

            return Get<TransferSummary[]>(url);
        }

        public ApiResponse<TransferDetail> GetTransfer(int transferID)
        {
            var url = MakeTransferUrl(_url, transferID);

            return Get<TransferDetail>(url);
        }


        public ApiResponse<TransferDetail> CreateTransfer(TransferDetail model)
        {
            var url = MakeTransferCollectionUrl(_url);

            return Post<TransferDetail, TransferDetail>(url, model);
        }

        public ApiResponse<object> UploadTransferFile(int transferID, string filename, int chunkSize)
        {
            var url = MakeTransferFilesUrl(_url, transferID);

            using (var fStream = new FileStream(filename, FileMode.Open, FileAccess.Read))
            {
                var totalChunks = Convert.ToInt32(Math.Ceiling(fStream.Length / (double)chunkSize));
                for (int i = 0; i < totalChunks; i++)
                {
                    var chunkStream = SplitFileStreamIntoChunk(fStream, chunkSize);

                    var chunkNum = i + 1;
                    var fileIdentifier = Path.GetFileName(filename);
                    ...
                    var chunk = new TransferChunk
                    {
                        ...
                        filePath = "/",
                        totalSize = fStream.Length
                    };

                    var hasBeenUploadedResponse = CheckIfChunkHasBeenUploaded(transferID, chunkNum, fileIdentifier);

                    if (hasBeenUploadedResponse.StatusCode != HttpStatusCode.OK)
                    {

                        var thisResponse = Post<object>(url, chunkStream, chunk);

                        if (thisResponse.StatusCode != HttpStatusCode.OK)
                            return thisResponse;
                    }
                }
            }
            return new ApiResponse<object> { Payload = null, StatusCode = HttpStatusCode.OK };
        }

        public ApiResponse<TransferValidationResult> CloseTransfer(int transferID)
        {
            var url = MakeTransferCloseUrl(_url, transferID);

            return Post<TransferValidationResult>(url);
        }

        public ApiResponse<object> CheckIfChunkHasBeenUploaded(int transferID, int chunkNum, string fileIdentifier)
        {
            var url = MakeTransferFilesChunkUrl(_url, transferID, chunkNum, fileIdentifier);

            return Get<object>(url);
        }

        public static string GetFileHashCodeForStream(Stream s)
        {
            String hashCode;
            using (MD5CryptoServiceProvider crypto = new MD5CryptoServiceProvider())
            {
                var hashBytesFile = crypto.ComputeHash(s);
                hashCode = BitConverter.ToString(hashBytesFile);
            }
            return hashCode;
        }
        ...
        private Stream SplitFileStreamIntoChunk(FileStream fs, int chunkSize)
        {
            var bytes = new byte[chunkSize + 1];
            var outStream = new MemoryStream();

            int size = fs.Read( bytes, 0, chunkSize);

            outStream.Write(bytes, 0, size);

            outStream.Position = 0; // reset stream to be at the beginning so the next process to read starts from the beginning.
        ...
        {
            return new Uri(baseUrl + "/" + TRANSFERS);
        }


        private static Uri MakeTransferUrl(string baseUrl, int transferID)
        {
            return new Uri(baseUrl + "/" + TRANSFERS + "/" + transferID);
        }

        private static Uri MakeTransferCloseUrl(string baseUrl, int transferID)
        {
            return new Uri(baseUrl + "/" + TRANSFERS + "/" + transferID + "/" + CLOSE);
        }

        private static Uri MakeTransferFilesUrl(string baseUrl, int transferID)
        {
            return new Uri(MakeTransferUrl(baseUrl, transferID) + "/" + FILES);
        }

        private static Uri MakeTransferFilesChunkUrl(string baseUrl, int transferID, int chunkNum, string fileIdentifier)
        {
            return new Uri(MakeTransferFilesUrl(baseUrl, transferID) + "/" + chunkNum + "/" + fileIdentifier);
        }
        ...
        public static void AddAuthToken(HttpClient client, TokenResponse token)
        {
        ...
        public ApiResponse<T> Post<T>(Uri url, Stream stream, TransferChunk model)
        {
            using (var form = new MultipartFormDataContent())
            {
                var len = stream.Length;
                ...
                using (var client = new HttpClient())
                {
                    ...
                    form.Add(new StringContent(model.chunk.ToString()), "chunk");
                    form.Add(new StringContent(model.chunkSize.ToString()), "chunkSize");
                    form.Add(new StringContent(model.totalSize.ToString()), "totalSize");
                    form.Add(new StringContent(model.totalChunks.ToString()), "totalChunks");
                    form.Add(new StringContent(model.fileIdentifier), "fileIdentifier");
                    form.Add(new StringContent(model.fileName), "fileName");
                    form.Add(new StringContent(model.filePath), "filePath");
                    form.Add(new ByteArrayContent(buffer, 0, buffer.Length), "filex", model.fileName);      // The token here is not important
        ...
        public ApiResponse<T> Post<T>(Uri url)
        {
            using (var client = new HttpClient())
            {
                AddAuthToken(client);
                _response = client.PostAsync(url, new StringContent("")).Result;
            }
            return ProcessResponse<T>(_response);
        }
        ...
        private ApiResponse<T> Post<T, TY>(Uri url, TY model)
        {
            using (var client = new HttpClient())
            {
                AddAuthToken(client);
        ...
        public static ApiResponse<T> ProcessResponse<T>(HttpResponseMessage response)
        {
            if (!response.IsSuccessStatusCode)
            {
                return new ApiResponse<T>
                {
                    ...
                    PayloadAsText = response.Content.ReadAsStringAsync().Result
